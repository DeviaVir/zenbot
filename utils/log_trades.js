...
module.exports = function container (get, set, clear) {
  ...
  return function log_trades (exchange, trades) {
    ...
    var ticker = trades.slice(0, c.log_trades_limit).map(function (trade) {
      ...
      return z(4, (trade.side === 'buy' ? 'SELL' : 'BUY') , ' ')[trade.side === 'buy' ? 'red' : 'green'] + ' ' + z(9, n(trade.size).format('0.000'), ' ')[trade.side === 'buy' ? 'red' : 'green'] + ' ' + config.asset + ' at '.grey + z(9, n(trade.price).format('0.000'), ' ').yellow + ' ' + config.currency.grey
    }).join(', ')
