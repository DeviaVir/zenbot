...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
    ...
    //=============================================
    // WbSocket objects needed for WS operation
    var zenTalk = talker.update(s)
    //var wsCommand = zenTalk.wsCommand
    var wsTalker  = zenTalk.wsTalker
    var lastPeriod = ''
    ...
    function syncBalance (cb) {
      //=========================================================
      // Experimental WS interface to show and modify params
      // Access with ./zentalk --connect <host:port>
      //=========================================================
      //console.log(s)
      zenTalk = talker.update(s)
      if (zenTalk.subscribed.period) {
        if (s.period.period_id !== lastPeriod) {
          var period = {'period': s.period}
          var msg = JSON.stringify(period) + '\n\n'
          wsTalker.emit('transmit',msg)
          lastPeriod = s.period.period_id
        }
      }
      //========================================================= */
    ...
    function executeOrder (trade) {
      ...
      if (s.buy_order) {
        if (so.mode === 'live' || trade.price <= Number(s.buy_order.price)) {
          ...
          //===============================================
          // 
          if (zenTalk.subscribed.lastTrade) {
            var trade = {'lastTrade': my_trade}
            var msg = JSON.stringify(trade) + '\n\n'
            wsTalker.emit('transmit',msg)
           }
          //=============================================== */
