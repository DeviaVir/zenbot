...
module.exports = function container (get, set, clear) {
  return function (s) {
    ...
    function fc (amt) {
      if (s.currency === 'BTC') return n(amt).format('0.00000000') + ' BTC'
        ...
        s.exchange[type](order, function (err, api_order) {
          ...
          function checkOrder () {
            ...
            s.exchange.getOrder({order_id: order.order_id}, function (err, api_order) {
              ...
              else if (api_order.status === 'open') {
                if (new Date().getTime() - order.local_time >= so.order_adjust_time * 10) {
                  console.error('\nopen order timed out. reordering...'.red)
                  if (Number(api_order.filled_size)) {
                    order.time = new Date().getTime()
                    order.size = api_order.filled_size
                    executeOrder(order)
                  }
                  cancelAndReorder()
                }
    ...
    function executeSignal (signal, _cb, size, waited_for_settlement) {
      var size, price
      var cb = function (err, order) {
        if (err) {
          ...
          if (_cb) {
            ...
            console.error('\n')
            if (err.desc) console.error(err.desc.red)
    ...
    function writeReport (is_progress, blink_off) {
      ...
      process.stdout.write(z(s.currency === 'BTC' ? 16 : 14, fc(s.period.close), ' ').yellow)
      if (s.lookback[0]) {
      ...
      process.stdout.write(z(6, s.period.volume > 99999 ? abbreviate(s.period.volume, 2) : n(s.period.volume).format('0'), ' ')[is_progress && blink_off ? 'cyan' : 'grey'])
      ...
      if (s.buy_order) {
        ...
        process.stdout.write(z(8, s.action, ' ')[s.action === 'bought' ? 'green' : 'red'])
      }
      else if (s.signal) {
        process.stdout.write(z(8, s.signal || '', ' ')[s.signal ? s.signal === 'buy' ? 'green' : 'red' : 'grey'])
      }
      else if (s.last_trade_worth && !s.buy_order && !s.sell_order) {
        process.stdout.write(z(7, pct(s.last_trade_worth), ' ')[s.last_trade_worth > 0 ? 'green' : 'red'])
      }
      else {
        process.stdout.write(z(8, '', ' '))
      }
      ...
      if (orig_capital) {
        ...
        var currency_col = n(s.balance.currency).format(s.currency === 'BTC' ? '0.00000' : '0.00') + ' ' + s.currency
