...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
    ...
    function fc (amt, omit_currency) {
      if (isFiat()) return n(amt).format('0.00') + (omit_currency ? '' : ' ' + s.currency)
    ...
    function writeReport (is_progress, blink_off) {
      ...
      process.stdout.write(z(isFiat() ? 14 : 16, fc(s.period.close, true), ' ').yellow + ' ' + s.product_id.grey)
      if (s.lookback[0]) {
