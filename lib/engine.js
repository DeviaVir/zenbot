...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
      ...
      if (s.my_trades.length) {
        ...
        if (!s.acted_on_stop) {
          if (last_trade.type === 'short'){
            if (do_sell_stop && s.short_stop && s.period.close > s.short_stop) {
              stop_signal = 'close'
              ...
              s.profit_stop_high = Math.min(s.profit_stop_high || s.period.close, s.period.close)
    ...
    /**
     * retrieve the balance on live (sim is calculated manually)
     * @param {*} cb 
     */
    ...
    /**
     * This function store the order and execute it on the exchange
     * @param {*} type 
     * @param {*} opts 
     * @param {*} cb 
     */
              ...
              function checkHold () {
                s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
                  ...
                  syncBalance(function () {
                    var on_hold
                    if (/buy|short/.test(type)) on_hold = n(s.balance.currency).subtract(s.balance.currency_hold || 0).value() < n(order.price).multiply(order.remaining_size).value()
                    else if(type ==='close') on_hold = false;//just closing the order ... no holding ;)
            ...
            s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
              ...
              if (new Date().getTime() - order.local_time >= so.order_adjust_time) {
                getQuote(function (err, quote) {
                  ...
                  else if(/short|close/.test(type)){
                    //Can't cancel or 'poll' those order ...
                  }else {
    ...
    /**
     * Get the quotes 
     * - from exchange for live
     * - from db for sim/train
     * @param {*} cb 
     */
    ...
    function executeSignal (signal, _cb, size, is_reorder, is_taker) {
      var size, price
      if(/buy|sell/.test(signal))delete s[signal=='buy'?'sell':'buy'+'_order'];
      if(/short|close/.test(signal))delete s[signal=='short'?'close':'short'+'_order'];
      /*for(k_remove in s){//Remove all the other orders ....
        if(/(buy|sell|short|close)_order/.test(k_remove) && k_remove!==signal+'_order'){delete s[k_remove];}
      }*/
      ...
      var cb = function (err, order) {
        if (!order) {
          ...
          else if(signal === 'short') delete s.short_order
          else if(signal === 'close') delete s.close_order
      ...
      syncBalance(function (err) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'short') {//short are made on market price
            ...
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || ('min_total' in s.product && s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              if (s.short_order && so.max_slippage_pct) {
                var slippage = n(price).subtract(s.short_order.orig_price).divide(s.short_order.orig_price).multiply(-100).value()
                if (so.max_slippage_pct && slippage > so.max_slippage_pct) {
                  var err = new Error('\nslippage protection')
                  err.desc = 'refusing to short at ' + fc(price) + ', slippage of ' + pct(slippage / 100)
                  return cb(err)
                }
              }
              if (n(s.balance.currency).subtract(s.balance.currency_hold || 0).value() < n(price).multiply(size).value() && s.balance.currency_hold > 0) {
                msg('short delayed: ' + pct(n(s.balance.currency_hold || 0).divide(s.balance.currency).value()) + ' of funds (' + fc(s.balance.currency_hold) + ') on hold')
                ...
                if (so.mode === 'live' || so.mode === 'paper') {
                  notify.pushMessage('Shorting ' + s.exchange.name.toUpperCase(), 'placing short order at ' + fc(price) + ', ' + fc(quote.bid - Number(price)) + ' under best bid\n')
                }
                doOrder()
              }
            }
            ...
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              var sell_loss = s.last_short_price ? (Number(price) - s.last_short_price) / s.last_short_price * 100 : null
              if (so.max_sell_loss_pct && sell_loss > so.max_sell_loss_pct) {
                var err = new Error('\nloss protection')
                err.desc = 'refusing to close at ' + fc(price) + ', close loss of ' + pct(sell_loss / 100)
                return cb(err)
              }
              else {
                if (s.close_order && so.max_slippage_pct) {
                  var slippage = n(s.close_order.orig_price).add(price).divide(price).multiply(100).value()
                  if (slippage > so.max_slippage_pct) {
                    var err = new Error('\nslippage protection')
                    err.desc = 'refusing to close at ' + fc(price) + ', slippage of ' + pct(slippage / 100)
                    return cb(err)
                  }
                }
                if (n(s.balance.asset).subtract(s.balance.asset_hold || 0).value() < n(size).value()) {
                  msg('close delayed: ' + pct(n(s.balance.asset_hold || 0).divide(s.balance.asset).value()) + ' of funds (' + fa(s.balance.asset_hold) + ') on hold')
                  return setTimeout(function () {
                    if (s.last_signal === signal) {
                      executeSignal(signal, cb, size, true)
                    }
                  ...
                  if (so.mode === 'live' || so.mode === 'paper') {
                    notify.pushMessage('Closing ' + s.exchange.name.toUpperCase(), 'placing close order at ' + fc(price) + ', ' + fc(Number(price) - quote.bid) + ' over best ask\n')
                  }
                  doOrder()
                }
              }
    ...
    /**
     * Attempt of generalization of executeOrder
     * Centralising calculation for balance and stuff
     * @param {*} trade 
     * @param {*} cOrderType short/buy/sell/close
     * @param {*} cBalance add/substract
     * @param {*} cAction name of the action
     * @param {*} cStopable can be stopped through sell_stop ?
     */
    function calcBalanceStop(trade,cOrderType,cBalance,cAction,cStopable=null){
      var price, fee = 0
      if (!so.order_type) {
        so.order_type = 'maker'
      }
      try{
        if (so.mode === 'live' || 
          (cBalance === 'add' && trade.price <= Number(s[cOrderType+'_order'].price)) ||
          (cBalance === 'substract' && trade.price >= Number(s[cOrderType+'_order'].price)) 
        ) {
          price = s[cOrderType+'_order'].price
          if (so.mode !== 'live') {
            price = n(s[cOrderType+'_order'].orig_price)[cBalance](n(s[cOrderType+'_order'].orig_price).multiply(so.avg_slippage_pct / 100)).format('0.00000000')
            s.balance.asset = n(s.balance.asset)[cBalance](s[cOrderType+'_order'].size).format('0.00000000')
            var total = n(price).multiply(s[cOrderType+'_order'].size)
            s.balance.currency = n(s.balance.currency)[cBalance==='add'?'subtract':'add'](total).format('0.00000000')
            if (so.order_type === 'maker') {
              if (s.exchange.makerFee) {
                fee = n(s[cOrderType+'_order'].size).multiply(s.exchange.makerFee / 100).value()
                s.balance.asset = n(s.balance.asset).subtract(fee).format('0.00000000')
              }
            }
            if (so.order_type === 'taker') {
              if (s.exchange.takerFee) {
                fee = n(s[cOrderType+'_order'].size).multiply(s.exchange.takerFee / 100).value()
                s.balance.asset = n(s.balance.asset).subtract(fee).format('0.00000000')
              }
            }
          }
          ...
          var my_trade = {
            ...
            time: trade.time,
            execution_time: trade.time - s[cOrderType+'_order'].orig_time,
            slippage: n(price).subtract(s[cOrderType+'_order'].orig_price).divide(s[cOrderType+'_order'].orig_price).value(),
            type: cOrderType,
            size: s[cOrderType+'_order'].orig_size,
            fee: fee,
            price: price,
          ...
          if (so.stats) {
            order_complete = '\n'+cOrderType+' order completed at ' + moment(trade.time).format('YYYY-MM-DD HH:mm:ss') + ':\n\n' + fa(my_trade.size) + ' at ' + fc(my_trade.price) + '\ntotal ' + fc(my_trade.size * my_trade.price) + '\n' + n(my_trade.slippage).format('0.0000%') + ' slippage (orig. price ' + fc(s[cOrderType+'_order'].orig_price) + ')\nexecution: ' + moment.duration(my_trade.execution_time).humanize() + '\n'
            console.log((order_complete).cyan)
            if (so.mode === 'live' || so.mode === 'paper') {
              notify.pushMessage(cOrderType+' ' + s.exchange.name.toUpperCase(), order_complete)
            }
          }
          s['last_'+cOrderType+'_price'] = my_trade.price
          delete s[cOrderType+'_stop']
          delete s.profit_stop
          delete s.profit_stop_high
          if(cStopable!==null){
            delete s[cStopable+'_order']
            delete s[cStopable+'_stop']
          }
          if (cStopable!==null&&!s.acted_on_stop && so[cStopable+'_stop_pct']) {
            s[cStopable+'_stop'] = n(price)[cStopable==='buy'?'add':'subtract'](n(price).multiply(so[cStopable+'_stop_pct'] / 100)).value()
          }
        }
     ...
     * -> should create a function that generalize this
     * Store the trade (like the order but internal to zenbot for status about the trade)
     * Calculate the balance for simulation
    ...
    function executeOrder (trade) {
      ...
      if(s.short_order){
        if (so.mode === 'live' || trade.price <= Number(s.short_order.price)) {
          price = s.short_order.price
          if (so.mode !== 'live') {
            price = n(s.short_order.orig_price).add(n(s.short_order.orig_price).multiply(so.avg_slippage_pct / 100)).format('0.00000000')
            s.balance.asset = n(s.balance.asset).add(s.short_order.size).format('0.00000000')
            var total = n(price).multiply(s.short_order.size)
            s.balance.currency = n(s.balance.currency).subtract(total).format('0.00000000')
            if (so.order_type === 'maker') {
              if (s.exchange.makerFee) {
                fee = n(s.short_order.size).multiply(s.exchange.makerFee / 100).value()
                s.balance.asset = n(s.balance.asset).subtract(fee).format('0.00000000')
              }
            }
            if (so.order_type === 'taker') {
              if (s.exchange.takerFee) {
                fee = n(s.short_order.size).multiply(s.exchange.takerFee / 100).value()
                s.balance.asset = n(s.balance.asset).subtract(fee).format('0.00000000')
              }
            }
          }
          ...
          var my_trade = {
            order_id: trade.order_id,
            time: trade.time,
            execution_time: trade.time - s.short_order.orig_time,
            slippage: n(price).subtract(s.short_order.orig_price).divide(s.short_order.orig_price).value(),
            type: 'short',
            size: s.short_order.orig_size,
            fee: fee,
            price: price,
            order_type: so.order_type || 'taker',
            cancel_after: so.cancel_after || 'day'
          ...
          s.my_trades.push(my_trade)
          if (so.stats) {
            order_complete = '\nshort order completed at ' + moment(trade.time).format('YYYY-MM-DD HH:mm:ss') + ':\n\n' + fa(my_trade.size) + ' at ' + fc(my_trade.price) + '\ntotal ' + fc(my_trade.size * my_trade.price) + '\n' + n(my_trade.slippage).format('0.0000%') + ' slippage (orig. price ' + fc(s.short_order.orig_price) + ')\nexecution: ' + moment.duration(my_trade.execution_time).humanize() + '\n'
            console.log((order_complete).cyan)
            if (so.mode === 'live' || so.mode === 'paper') {
              notify.pushMessage('Short ' + s.exchange.name.toUpperCase(), order_complete)
            }
          }
          s.last_short_price = my_trade.price
          delete s.short_order
          delete s.short_stop
          delete s.sell_stop
          if (!s.acted_on_stop && so.sell_stop_pct) {
            s.short_stop = n(price).subtract(n(price).multiply(so.sell_stop_pct / 100)).value()
          }
          delete s.profit_stop
          delete s.profit_stop_high
        }
      ...
      }else if (s.close_order) {
        if (so.mode === 'live' || trade.price >= s.close_order.price) {
          price = s.close_order.price
          if (so.mode !== 'live') {
            price = n(s.close_order.orig_price).subtract(n(s.close_order.orig_price).multiply(so.avg_slippage_pct / 100)).format('0.00000000')
            s.balance.asset = n(s.balance.asset).subtract(s.close_order.size).value()
            var total = n(price).multiply(s.close_order.size)
            s.balance.currency = n(s.balance.currency).add(total).value()
            if (so.order_type === 'maker') {
              if (s.exchange.makerFee) {
                fee = n(s.close_order.size).multiply(s.exchange.makerFee / 100).multiply(price).value()
                s.balance.currency = n(s.balance.currency).subtract(fee).format('0.00000000')
              }
            }
            if (so.order_type === 'taker') {
              if (s.exchange.takerFee) {
                fee = n(s.close_order.size).multiply(s.exchange.takerFee / 100).multiply(price).value()
                s.balance.currency = n(s.balance.currency).subtract(fee).format('0.00000000')
              }
            }
          }
          ...
          var my_trade = {
            order_id: trade.order_id,
            time: trade.time,
            execution_time: trade.time - s.close_order.orig_time,
            slippage: n(s.close_order.orig_price).subtract(price).divide(price).value(),
            type: 'close',
            size: s.close_order.orig_size,
            fee: fee,
            price: price,
            order_type: so.order_type
          }
          s.my_trades.push(my_trade)
          if (so.stats) {
            order_complete = '\close order completed at ' + moment(trade.time).format('YYYY-MM-DD HH:mm:ss') + ':\n\n' + fa(my_trade.size) + ' at ' + fc(my_trade.price) + '\ntotal ' + fc(my_trade.size * my_trade.price) + '\n' + n(my_trade.slippage).format('0.0000%') + ' slippage (orig. price ' + fc(s.close_order.orig_price) + ')\nexecution: ' + moment.duration(my_trade.execution_time).humanize() + '\n'
            console.log((order_complete).cyan)
            if (so.mode === 'live' || so.mode === 'paper') {
                notify.pushMessage('Closed ' + s.exchange.name.toUpperCase(), order_complete)
            }
          }
          s.last_sell_price = my_trade.price
          delete s.close_order
          delete s.short_stop
          delete s.profit_stop
          delete s.profit_stop_high
        }
    ...
    /**
     * Prices adjustement (order_adjust_time)
     * @param {*} trade 
     */
    function adjustBid (trade) {
      ...
      //close and short can't be adjusted so nothing to do about it
    }
    ...
    function writeReport (is_progress, blink_off) {
      ...
      else if (s.short_order) {
        process.stdout.write(z(9, 'shorting', ' ').cyan)
      }
      else if (s.close_order) {
        process.stdout.write(z(9, 'closing', ' ').blue)
      }
      else if (s.action) {
        process.stdout.write(z(9, s.action, ' ')[/bought|shorted/.test(s.action) ? 'green' : 'red'])
      }
      else if (s.signal) {
        process.stdout.write(z(9, s.signal || '', ' ')[s.signal ? /buy|short/.test(s.signal) ? 'green' : 'red' : 'grey'])
      }
