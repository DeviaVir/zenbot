...
module.exports = function container (get, set, clear) {
  return function (s) {
      ...
      if (s.my_trades.length) {
        ...
        if (!s.acted_on_stop) {
          if (last_trade.type === 'buy') {
            if (s.sell_stop && s.period.close < s.sell_stop && (!so.sell_stop_max_pct || (s.last_trade_worth * -100 < so.sell_stop_max_pct))) {
              stop_signal = 'sell'
    ...
    function executeSignal () {
      var size, price
      if (s.signal === 'buy') {
        size = s.balance.currency / s.period.close
        if (size >= 0.01)  {
    ...
    function writeReport () {
      ...
      process.stdout.write(z(8, fc(s.period.close), ' ').white)
      if (s.lookback[0]) {
        var diff = s.period.close - s.lookback[0].close
        process.stdout.write(z(8, fc(diff), ' ')[diff >= 0 ? 'green' : 'red'])
      }
      else {
      ...
      if (s.action) {
        ...
        process.stdout.write(z(9, pct(s.last_trade_worth), ' ')[s.last_trade_worth > 0 ? 'green' : 'red'])
      }
      else {
        process.stdout.write(z(10, '', ' '))
      }
      process.stdout.write(z(9, fa(s.balance.asset), ' ').white)
      process.stdout.write(z(10, fc(s.balance.currency), ' ').yellow)
