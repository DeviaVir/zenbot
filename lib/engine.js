...
module.exports = function container (get, set, clear) {
  return function (s) {
      ...
      if (s.signal === 'buy') {
        ...
        if (size >= 0.01)  {
          ...
          if (so.max_sell_loss_pct && sell_loss > so.max_sell_loss_pct) {
            console.error('\nrefusing to sell at', fc(price), 'sell loss of', pct(sell_loss / 100) + '\n')
          }
          else {
            s.sell_order = {
              size: size,
    ...
    function writeReport (is_progress) {
      ...
      process.stdout.write(moment(is_progress ? new Date().getTime() : s.period.time).format('YYYY-MM-DD HH:mm:ss')[is_progress ? 'bgBlue' : 'grey'])
