    ...
    function executeSignal (signal, _cb, size, is_reorder, is_taker) {
      ...
      syncBalance(function (err) {
        ...
        getQuote(function (err, quote) {
		  ...
		  // todo : find my way through this shite code and adjust so I can run multiple bots on a single balance!
		  // buy_pct needs to be buy_amount 0.1, this will include gains/loses made trading this pair
		  // or I need to run a demon to update the balance on a schedule, to gains get rolled in
		  // issue- > https://github.com/DeviaVir/zenbot/issues/174 = Set a max amount ;) --buy_amount_max
          if (signal === 'buy') {
