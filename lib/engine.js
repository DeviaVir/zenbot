...
module.exports = function container (get, set, clear) {
  return function (s) {
        ...
        s.exchange[type](order, function (err, api_order) {
          ...
          function checkOrder () {
            if (!s[type + '_order']) {
              ...
              err.order = order
              s.exchange.cancelOrder({order_id: order.id}, function () {
                cb(err)
              })
              return
            }
    ...
    function executeSignal (signal, cb) {
      var size, price
      cb || (cb = function (err) {
        if (err) {
      ...
      if (s.buy_order) {
        if (so.mode === 'live' || trade.price <= Number(s.buy_order.price)) {
          ...
          var my_trade = {
            ...
            slippage: n(s.buy_order.price).subtract(s.buy_order.orig_price).divide(s.buy_order.orig_price).value(),
            type: 'buy',
            size: n(s.buy_order.size).value(),
            price: n(price).value()
          }
          s.my_trades.push(my_trade)
          console.log(('\nbuy order completed at ' + moment(trade.time).format('YYYY-MM-DD HH:mm:ss') + ':\n\n' + fa(my_trade.size) + ' at ' + fc(my_trade.price) + '\ntotal ' + fc(my_trade.size * my_trade.price) + '\n' + n(my_trade.slippage).format('0.0000%') + ' slippage\n' + n(my_trade.execution_time / 60000).format('0.00') + ' min execution\n').cyan)
        ...
        if (so.mode === 'live' || trade.price >= s.sell_order.price) {
          ...
          var my_trade = {
            ...
            slippage: n(s.sell_order.orig_price).subtract(s.sell_order.price).divide(s.sell_order.price).value(),
            type: 'sell',
    ...
    function writeReport (is_progress) {
      process.stdout.clearLine()
      process.stdout.cursorTo(0)
      process.stdout.write(moment(is_progress ? s.period.close_time : s.period.time + 3600000).format('YYYY-MM-DD HH:mm:ss')[is_progress ? 'bgBlue' : 'grey'])
      ...
      var profit = s.start_capital ? (consolidated - s.start_capital) / s.start_capital : 0
