  ...
  return function (s) {
    ...
    function syncBalance (cb) {
      ...
      s.exchange.getBalance(s, function (err, balance) {
        ...
        s.balance = balance
        if (!s.start_capital) {
          s.exchange.getQuote({product_id: so.product_id}, function (err, quote) {
            if (err) return cb(err)
        ...
        s.exchange[type](order, function (err, api_order) {
          ...
          function checkOrder () {
            if (!s[type + '_order']) {
              ...
              err.order = order
              s.exchange.cancelOrder({order_id: order.id}, function (cancel_err) {
                if (cancel_err) {
            ...
            s.exchange.getOrder({order_id: order.id}, function (err, api_order) {
              if (err) return cb(err)
              order.status = api_order.status
              ...
              if (new Date().getTime() - order.local_time >= so.order_adjust_time) {
                s.exchange.cancelOrder({order_id: order.id}, function (err) {
                  if (err) {
    ...
    function executeSignal (signal, cb, waited_for_settlement) {
      var size, price
      cb || (cb = function (err) {
        if (err) {
          if (err.desc || err.body) {
      ...
      syncBalance(function (err) {
        ...
        if (signal === 'buy' && Number(s.balance.currency_hold) > 0) {
          if (waited_for_settlement) {
            console.error(pct(n(s.balance.currency_hold).divide(s.balance.currency).value()) + ' of funds (' + fc(s.balance.currency_hold) + ') still on hold, giving up')
            return cb(null, null)
          }
          console.error(pct(n(s.balance.currency_hold).divide(s.balance.currency).value()) + ' of funds (' + fc(s.balance.currency_hold) + ') on hold, waiting 10s for settlement')
          ...
          if (waited_for_settlement) {
            console.error(pct(n(s.balance.asset_hold).divide(s.balance.asset).value()) + ' of funds (' + fa(s.balance.asset_hold) + ') still on hold, giving up')
            return cb(null, null)
          }
          console.error(pct(n(s.balance.asset_hold).divide(s.balance.asset).value()) + ' of funds (' + fa(s.balance.asset_hold) + ') on hold, waiting 10s for settlement')
          return setTimeout(function () {
            executeSignal(signal, cb, true)
          }, 10000)
        }
    ...
    function writeReport (is_progress, blink_off) {
      if (is_progress && typeof blink_off === 'undefined') {
        setTimeout(function () {
      ...
      process.stdout.write(z(14, n(s.period.volume).format('0.00'), ' ').grey)
