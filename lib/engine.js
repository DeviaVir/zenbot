...
module.exports = function container (get, set, clear) {
  ...
  return function (s) {
              ...
              if (new Date().getTime() - order.local_time >= so.order_adjust_time) {
                getQuote(function (err, quote) {
                  ...
                  if (type === 'buy') {
                    ...
                    marked_price = n(mid_price).subtract(n(mid_price).multiply(so.markup_pct / 100)).format('0.00000000', Math.floor)
                    ...
                    marked_price = n(mid_price).add(n(mid_price).multiply(so.markup_pct / 100)).format('0.00000000', Math.ceil)
                    if (n(order.price).value() > marked_price) {
                      cancelAndReorder()
                    }
                    else {
    ...
    function executeSignal (signal, _cb, size, is_reorder) {
      var size, price
      ...
      syncBalance(function (err) {
        ...
        if (signal === 'buy' && Number(s.balance.currency_hold) > 0) {
          msg('buy delayed: ' + pct(n(s.balance.currency_hold).divide(s.balance.currency).value()) + ' of funds (' + fc(s.balance.currency_hold) + ') on hold')
          ...
          msg('sell delayed: ' + pct(n(s.balance.asset_hold).divide(s.balance.asset).value()) + ' of funds (' + fa(s.balance.asset_hold) + ') on hold')
          return setTimeout(function () {
            if (s.last_signal === signal) {
              executeSignal(signal, cb, size, true)
            }
        ...
        getQuote(function (err, quote) {
          ...
          var mid_price = n(quote.ask).add(quote.bid).divide(2)
          if (signal === 'buy') {
            price = n(mid_price).subtract(n(mid_price).multiply(so.markup_pct / 100)).format(String(s.product.increment), Math.floor)
            if (!size) {
            ...
            price = n(mid_price).add(n(mid_price).multiply(so.markup_pct / 100)).format(String(s.product.increment), Math.ceil)
            if (!size) {
