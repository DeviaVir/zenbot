              ...
              function checkHold () {
                s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
                  ...
                  syncBalance(function () {
                    var on_hold
                    if (type === 'buy') on_hold = n(s.balance.currency).subtract(s.balance.currency_hold).value() < n(order.price).multiply(order.size).value()
                    else on_hold = n(s.balance.asset).subtract(s.balance.asset_hold).value() < n(order.size).value()
                    if (on_hold) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy') {
            ...
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              if (n(s.balance.currency).subtract(s.balance.currency_hold).value() < n(price).multiply(size).value()) {
            ...
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              if (so.max_sell_loss_pct && sell_loss > so.max_sell_loss_pct) {
                ...
                if (n(s.balance.asset).subtract(s.balance.asset_hold).value() < n(size).value()) {
