...
module.exports = function container (get, set, clear) {
  return function (s) {
    ...
    function fc (amt) {
      if (isFiat()) return n(amt).format('0.00') + ' ' + s.currency
      var str = n(amt).format('0.00000000')
      ...
      if (s.buy_order) {
        if (so.mode === 'live' || trade.price <= Number(s.buy_order.price)) {
          ...
          if (so.mode !== 'live') {
            ...
            s.balance.currency = n(s.balance.currency).subtract(n(price).multiply(s.buy_order.size)).format('0.00000000')
          }
          s.action = 'bought'
          var my_trade = {
    ...
    function writeReport (is_progress, blink_off) {
      ...
      process.stdout.write(z(isFiat() ? 14 : 16, fc(s.period.close), ' ').yellow)
      if (s.lookback[0]) {
