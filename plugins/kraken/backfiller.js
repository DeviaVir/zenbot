  ...
  return function mapper () {
    ...
    products.forEach(function (product) {
      ...
      function getNext () {
        ...
        var uri = x.rest_url + '/public/Trades?pair=' + product.id + '&limit=' + x.backfill_limit + (s.backfiller_id ? '&since=' + s.backfiller_id : '')
        //comment
        //get('logger').info(z(c.max_slug_length, 'backfiller GET', ' '), uri.grey)
        get('logger').info("HERE", uri)
        request(uri, {headers: {'User-Agent': USER_AGENT}}, function (err, resp, result) {
          ...
          get('logger').info("KRAKEN data", result.result.XXBTZUSD)

          withResult(result.result.XXBTZUSD[0])
        })
      }
      getNext()
    })
