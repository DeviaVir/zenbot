...
module.exports = function container (get, set, clear) {
  var bot = get('bot')
  if (bot.learned) {
    get('motley:db.mems').destroy('learned', function (err, destroyed) {
      if (err) throw err
      else {
  ...
  if (bot.rs) {
    get('motley:db.mems').destroy(constants.product_id, function (err, destroyed) {
      if (err) throw err
      console.log(JSON.stringify(destroyed || null, null, 2))
      process.exit()
    })
  ...
  if (bot.balance) {
    get('motley:db.mems').load('learned', function (err, learned) {
      ...
      get('motley:db.mems').save(learned, function (err, saved) {
        if (err) throw err
