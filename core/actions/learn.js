...
//  }
  ...
  //var bot = {}
  var c = get('config')
  var learner = require('../../../../learner/learner')
  ...
  var defaults = require('../../../../learner/defaults.json'), last_result
  ...
  get('learned').load('learned', function (err, rs) {
    if (err) throw err
    if (!rs) {
      rs = {
        id: 'learned',
    ...
    ;(function doNext () {
      ...
      proc.on('exit', function (code) {
        ...
        if (param && result.fitness > rs.best_fitness) {
          ...
          fs.writeFileSync(path.resolve(__dirname, '../../../../', 'learner', 'defaults.json'), JSON.stringify(rs.best_params, null, 2))
          //share()
        }
        else if (param) {
          process.stderr.write('\n\n\n\n')
