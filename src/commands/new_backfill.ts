...
import backfillFunction from './backfill/backfill.function'
...
export default (program, conf) => {
  program
    ...
    .action(function(selector, cmd) {
      ...
      if (exchange !== undefined) {
        var msg = "Hitting up the exchange '" + exchangeName + "' for trades within the past " + so.days + ' day'
        if (so.days > 1) {
          msg += 's.'
        } else {
          msg += '.'
        ...
        var targetTime = tb(new Date().getTime())
          .resize('1d')
          .subtract(so.days)
          .toMilliseconds()
