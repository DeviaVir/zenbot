...
var fa_defaultIndicators = ['CCI', 'MACD', 'RSI', 'SAR', 'Stochastic']
...
export default (program, conf) => {
  program
    ...
    .option('--days_training <days>', 'set duration of training dataset by day count', Number, conf.days)
    .option(
      '--days_test <days>',
    ...
    .option(
      ...
      'population count within one generation (default: ' + fa_getTrainOptions().populationCount + ')',
      Number
    )
    .option(
      '--generationCount <value>',
      'generation count (default: ' + fa_getTrainOptions().generationCount + ')',
      Number
    )
    .option(
      '--selectionAmount <value>',
      'how many chromosomes shall be selected from the old generation when constructing a new one (default: ' +
        fa_getTrainOptions().selectionAmount +
        ')',
      Number
    )
    .option(
      '--leafValueMutationProbability <value>',
      'leaf value mutation probability (default: ' + fa_getTrainOptions().leafValueMutationProbability + ')',
      Number
    )
    .option(
      '--leafSignMutationProbability <value>',
      'leaf sign mutation probability (default: ' + fa_getTrainOptions().leafSignMutationProbability + ')',
      Number
    )
    .option(
      '--logicalNodeMutationProbability <value>',
      'logical node mutation probability (default: ' + fa_getTrainOptions().logicalNodeMutationProbability + ')',
      Number
    )
    .option(
      '--leafIndicatorMutationProbability <value>',
      'leaf indicator mutation probability (default: ' + fa_getTrainOptions().leafIndicatorMutationProbability + ')',
      Number
    )
    .option(
      '--crossoverProbability <value>',
      'crossover probability (default: ' + fa_getTrainOptions().crossoverProbability + ')',
      Number
    )
    .option(
      '--indicators <value>',
      'comma separated list of TA-lib indicators (default: ' +
        fa_defaultIndicators.toString() +
        ', available: ' +
        fa_availableIndicators.toString() +
        ')',
      String
    )

    .action(function(selector, cmd) {
      var s: Record<string, any> = { options: minimist(process.argv) }
      var so = s.options
      delete so._
      if (cmd.conf) {
      ...
      if (so.indicators) {
        so.indicators.split(',').forEach(function(indicator) {
          if (!fa_availableIndicators.includes(indicator)) unknownIndicators.push(indicator)
        })
      }
      if (unknownIndicators.length > 0) {
        console.error(
          'ERROR: The following indicators are not in forex.analytics: '.red + unknownIndicators.toString().yellow
        )
      ...
      var query_start = so.start_training
      ...
      function trainingDone(strategy, lastPeriod) {
        ...
        var trainingArgs = [
          ...
          '--start',
          moment(so.start_training).format('YYYYMMDDHHmm'),
          '--end',
          moment(so.end_training).format('YYYYMMDDHHmm'),
          '--period',
          so.period_length,
          '--filename',
          path.resolve(__dirname, '..', tempModelFile) + '-simTrainingResult.html',
        ]
          ...
          if (so.days_test > 0) {
            ...
            console.log(
              '\nRunning simulation on test data from ' +
                moment(so.end_training).format('YYYY-MM-DD HH:mm:ss ZZ') +
                ' to ' +
                moment(endTest).format('YYYY-MM-DD HH:mm:ss ZZ') +
                ' (' +
                so.days_test +
                ' days).\n'
            )
            ...
            var testArgs = [
              ...
              moment(so.end_training).format('YYYYMMDDHHmm'),
              '--end',
              moment(endTest).format('YYYYMMDDHHmm'),
              '--period',
              so.period_length,
              '--filename',
              path.resolve(__dirname, '..', tempModelFile) + '-simTestResult.html',
            ]
            ...
            testSimulation.on('exit', function(code, signal) {
              ...
              fs.renameSync(
                path.resolve(__dirname, '..', tempModelFile) + '-simTrainingResult.html',
                path.resolve(__dirname, '..', finalModelFile) + '-simTrainingResult.html'
              )
              fs.renameSync(
                path.resolve(__dirname, '..', tempModelFile) + '-simTestResult.html',
                path.resolve(__dirname, '..', finalModelFile) + '-simTestResult.html'
              )
              fs.unlinkSync(path.resolve(__dirname, '..', tempModelFile))
      ...
      function createStrategy(candlesticks) {
        var bar = new ProgressBar('Training [:bar] :percent :etas - Fitness: :fitness', {
          width: 80,
          total: fa_getTrainOptions(so).generationCount,
          incomplete: ' ',
        })

        return analytics.findStrategy(candlesticks, fa_getTrainOptions(so), function(
          strategy,
          fitness /*, generation*/
        ) {
          bar.tick({
