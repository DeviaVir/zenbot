...
export default (program, conf) => {
  program
    ...
    .option(
      ...
      'for paper trading, amount of start capital in currency',
      Number,
      conf.currency_capital
    )
    .option(
      '--asset_capital <amount>',
      'for paper trading, amount of start capital in asset',
      Number,
      conf.asset_capital
    )
    .option(
      '--avg_slippage_pct <pct>',
      'avg. amount of slippage to apply to paper trades',
      Number,
      conf.avg_slippage_pct
    )
    .option('--buy_pct <pct>', 'buy with this % of currency balance', Number, conf.buy_pct)
    .option(
      '--deposit <amt>',
      'absolute initial capital (in currency) at the bots disposal (previously --buy_max_amt)',
      Number,
      conf.deposit
    )
    ...
    .option(
      ...
      Number,
      conf.profit_stop_enable_pct
    )
    .option(
      '--profit_stop_pct <pct>',
      'maintain a trailing stop this % below the high-water mark of profit',
      Number,
      conf.profit_stop_pct
    )
    ...
    .option(
      '--keep_lookback_periods <amount>',
      'Keep this many lookback periods max. ',
      Number,
      conf.keep_lookback_periods
    )
    .option(
      '--exact_buy_orders',
      'instead of only adjusting maker buy when the price goes up, adjust it if price has changed at all'
    )
    .option(
      '--exact_sell_orders',
      'instead of only adjusting maker sell when the price goes down, adjust it if price has changed at all'
    )
    ...
    .action(function(selector, cmd) {
      ...
      var s: Record<string, any> = { options: JSON.parse(JSON.stringify(raw_opts)) }
      var so = s.options
      if (so.run_for) {
      ...
      function listOptions() {
        ...
        process.stdout.write(
          [
            ...
            z(
              31,
              so.mode === 'paper' ? 'avg. '.grey + so.avg_slippage_pct + '%' : 'max '.grey + so.max_slippage_pct + '%',
              ' '
            ),
            z(
              20,
      ...
      function printTrade(quit, dump?, statsonly = false) {
        ...
        var profit = s.start_capital
          ...
          : n(0)
        var buy_hold = s.start_price
          ? n(s.period.close).multiply(n(s.start_capital).divide(s.start_price))
          : n(tmp_balance)
          ...
          trades
            ...
            .toArray(function(err, trades) {
              ...
              if (trades.length && so.use_prev_trades) {
                let prevOpts: Record<string, any> = {
                  query: {
                    selector: so.selector.normalized,
                  },
        ...
        function saveSession() {
          engine.syncBalance().then(function() {
            ...
            if (s.period) {
              ...
              b._id = b.id
              b.consolidated = n(s.balance.asset)
