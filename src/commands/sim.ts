...
export default (program, conf) => {
  program
    ...
    .option(
      '--backtester_generation <generation>',
      'creates a json file in simulations with the generation number',
      Number,
      -1
    )
    ...
    .action(function(selector, cmd) {
      ...
      function exitSim() {
        ...
        s.balance.currency = n(s.net_currency)
        ...
        options_output.simresults.vs_buy_hold =
          n(s.balance.currency)
            .subtract(buy_hold)
            .divide(buy_hold)
            .value() * 100.0
        ...
        if (so.backtester_generation >= 0) {
          fs.writeFileSync(
            path.resolve(
              ...
              'simulations',
              'sim_' +
                so.strategy.replace('_', '') +
                '_' +
                so.selector.normalized.replace('_', '').toLowerCase() +
                '_' +
                so.backtester_generation +
                '.json'
            ),
            options_json,
            { encoding: 'utf8' }
