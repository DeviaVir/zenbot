...
module.exports = function(conf) {
      ...
      do {
        if (direction == 'backward') {
          if (ranges[curr].from === ranges[next].to + 1) {
            ...
            if (next + 1 >= ranges.length) newRanges.push(ranges[next])
  ...
  theService.ping = (obj) => {
    ...
    if (rtn !== undefined) {
      if (direction == 'backward') theFarthestIdInRange = rtn.from
      else theFarthestIdInRange = rtn.to
    } else {
  ...
  theService.flush = (cb) => {
    if (ranges.length > 0) {
      collectionServiceInstance
        .getResumeMarkers()
        .deleteMany({ selector: selector.normalized }, function(/*err, data*/) {
          if (ranges.length > 1) {
