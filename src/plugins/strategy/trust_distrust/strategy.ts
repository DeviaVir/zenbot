  ...
  onPeriod: function(s, cb) {
    ...
    if (s.trust_distrust_last_action !== 'sell') {
      ...
      if (
        ...
        s.period.high < s.trust_distrust_highest - (s.trust_distrust_highest / 100) * s.options.sell_threshold_max
      ) {
        // we panic sell
        s.signal = 'sell'
    ...
    if (
      s.options.greed > 0 &&
      s.period.high > s.trust_distrust_start_greed + (s.trust_distrust_start_greed / 100) * s.options.greed
    ) {
    ...
    if (s.trust_distrust_last_action !== 'buy') {
      if (
        ...
        s.period.high > s.trust_distrust_lowest + (s.trust_distrust_lowest / 100) * s.options.buy_threshold
      ) {
