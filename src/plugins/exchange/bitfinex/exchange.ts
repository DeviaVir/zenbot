...
export default (conf) => {
  ...
  function wsUpdateTrades(pair, trades) {
    ...
    if (ws_trades.length > 1010) ws_trades.shift()
  }
  ...
  function wsMessage(message) {
    ...
    if (message[0] != 'undefined') ws_hb[message[0]] = Date.now()
  }
  ...
  function wsUpdateOrder(ws_order) {
    ...
    order.filled_size = n(ws_order[7])
      .subtract(ws_order[6])
      .format('0.00000000')
      ...
      heartbeat_interval = setInterval(function() {
        if (ws_hb[event.chanId]) {
          ...
          if (timeoutThreshold > ws_hb[event.chanId]) {
            console.warn(
              (
                "\nWebSockets Warning: No message on channel 'trade' within " +
                ws_timeout / 1000 +
    ...
    switch (conf.bitfinex.wallet) {
      case 'margin':
        try {
          ...
          if (so.debug) {
            console.warn(e)
            console.warn(
              "\nWebSockets Warning: Cannot send 'calc' for getBalance update (maybe connection not open?).".red +
      ...
      case 'exchange':
        try {
          ...
          ws_client.send([0, 'calc', null, [['wallet_exchange_' + opts.currency], ['wallet_exchange_' + opts.asset]]])
        } catch (e) {
          if (so.debug) {
            console.warn(e)
            console.warn(
    ...
    getBalance: function(opts, cb) {
      ...
      if (Object.keys(ws_balance).length === 0) {
        if (so.debug && ws_connected === true) {
          console.warn(
            'WebSockets Warning: Waiting for initial websockets snapshot.'.red +
              ' Retrying in ' +
              (ws_retry / 1000 + ' seconds').yellow +
              '.'
          )
        }
      ...
      if (ws_walletCalcDone[opts.asset] === false && ws_walletCalcDone[opts.currency] === false) {
        ...
        balance.currency =
          ws_balance[opts.currency] && ws_balance[opts.currency].balance
            ? n(ws_balance[opts.currency].balance).format('0.00000000')
            : n(0).format('0.00000000')
        balance.asset =
          ws_balance[opts.asset] && ws_balance[opts.asset].balance
            ? n(ws_balance[opts.asset].balance).format('0.00000000')
            : n(0).format('0.00000000')

        balance.currency_hold =
          ws_balance[opts.currency] && ws_balance[opts.currency].available
            ? n(ws_balance[opts.currency].balance)
                .subtract(ws_balance[opts.currency].available)
                .format('0.00000000')
            : n(0).format('0.00000000')
        balance.asset_hold =
          ws_balance[opts.asset] && ws_balance[opts.asset].available
            ? n(ws_balance[opts.asset].balance)
                .subtract(ws_balance[opts.asset].available)
                .format('0.00000000')
            : n(0).format('0.00000000')
    ...
    cancelOrder: function(opts, cb) {
      ...
      try {
        ...
        if (so.debug) {
          console.warn(e)
          console.warn(
            '\nWebSockets Warning: Cannot send cancelOrder (maybe connection not open?).'.red +
              ' Retrying in ' +
              (ws_retry / 1000 + ' seconds').yellow +
              '.'
          )
    ...
    trade: function(action, opts, cb) {
      ...
      try {
        ...
        if (so.debug) {
          console.warn(e)
          console.warn(
            '\nWebSockets Warning: Cannot send trade (maybe connection not open?).'.red +
              " Orders are sensitive, we're marking this one as rejected and will not just repeat the order automatically."
                .yellow
          )
        }
