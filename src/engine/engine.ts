...
export default function(s, conf) {
    ...
    s.exchange.getBalance({ currency: s.currency, asset: s.asset }, function(err, balance) {
      ...
      getQuote(function(err, quote) {
        ...
        if (!s.start_capital) {
          ...
          s.start_capital = n(s.balance.deposit)
            .add(s.asset_capital)
            .value()
          s.real_capital = n(s.balance.currency)
            .add(s.asset_capital)
            .value()
          ...
          s.net_currency = n(s.net_currency)
            .add(post_currency)
            .value()
        }
    ...
    syncBalance(function(err, { quote }) {
      ...
      if (signal === 'buy') {
        ...
        if (so.use_fee_asset) {
          ...
          so.order_type === 'maker' &&
          (buy_pct + s.exchange.takerFee < 100 || !s.exchange.makerBuy100Workaround)
        ) {
          fee = s.exchange.makerFee
        } else {
          fee = s.exchange.takerFee
        }
        trade_balance = n(s.balance.deposit)
          .divide(100)
          .multiply(buy_pct)
        ...
        if (buy_pct + fee < 100) {
          size = n(tradeable_balance)
            .divide(price)
            .format(s.product.asset_increment ? s.product.asset_increment : '0.00000000')
        } else {
        ...
        if (
          (s.product.min_size && Number(size) >= Number(s.product.min_size)) ||
          ('min_total' in s.product &&
            s.product.min_total &&
            n(size)
              .multiply(price)
              .value() >= Number(s.product.min_total))
        ) {
          ...
          let buy_loss = latest_low_sell
            ? ((latest_low_sell.price - Number(price)) / latest_low_sell.price) * -100
            : null
        ...
        if (
          ...
          let sell_loss = latest_high_buy
            ? ((Number(price) - latest_high_buy.price) / latest_high_buy.price) * -100
            : null
  ...
  function executeOrder(trade, type) {
    ...
    if (type === 'buy') s.buy_order = trade
    else if (type === 'sell') s.sell_order = trade

    if (s.buy_order) {
      price = s.buy_order.price
      if (so.order_type === 'maker') {
        if (s.exchange.makerFee) {
          fee = n(s.buy_order.size)
            .multiply(s.exchange.makerFee / 100)
            .value()
        }
      }
      if (so.order_type === 'taker') {
        if (s.exchange.takerFee) {
          fee = n(s.buy_order.size)
            .multiply(s.exchange.takerFee / 100)
            .value()
        }
      }
      s.action = 'bought'
      ...
      let my_trade = {
        ...
        slippage: n(s.sell_order.orig_price)
          .subtract(price)
          .divide(price)
          .value(),
        type: 'sell',
      ...
      moment(
        is_progress
          ...
          : tb(s.period.time)
              ...
              .add(1)
              .toMilliseconds()
      ).format('YYYY-MM-DD HH:mm:ss')[is_progress && !blink_off ? 'bgBlue' : 'grey']
    )
