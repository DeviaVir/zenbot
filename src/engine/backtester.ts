...
let processOutput = function(output, taskStrategyName, pheno) {
  ...
  let tFileName = path.resolve(
    ...
    'simulations',
    'sim_' +
      taskStrategyName.replace('_', '') +
      '_' +
      selector.toLowerCase().replace('_', '') +
      '_' +
      pheno.backtester_generation +
      '.json'
  )
...
let monitor: Record<string, any> = {
  ...
  reportStatus: function() {
    ...
    this.phenotypes.forEach(function(p) {
      if ('sim' in p) {
        if (Object.keys(p.sim).length === 0) {
          ...
          if (!p.command || !p.command.result) simsErrored++
    ...
    inProgress.forEach(function(p) {
      ...
      if (currentTime && c.queryStart && c.queryEnd) {
        ...
        if (homeStretchMode)
          inProgressStr.push(
            `${(c.iteration + ':').gray} ${(percentage * 100).toFixed(1)}% ETA: ${monitor.distanceOfTimeInWords(
              eta,
              now
            )}`
          )
        else inProgressStr.push(`${(c.iteration + ':').gray} ${(percentage * 100).toFixed(1)}%`)
      }
    })
    ...
    process.stdout.write(
      `Done: ${simsDone.toString().green}, Active: ${simsActive.toString().yellow}, Remaining: ${
        simsRemaining.toString().gray
      }, `
    )
    if (simsErrored > 0) process.stdout.write(`Errored: ${simsErrored.toString().red}, `)
    ...
    process.stdout.write(
      `Best Balance(${(bestP ? bestP.command.iteration.toString() : '?')[bestBColor]}): ${bestBalanceString}`
    )

    if (inProgressStr.length > 0) {
      if (!homeStretchMode)
        process.stdout.write(
          `, Slowest(${slowestP.command.iteration.toString().yellow}) ETA: ${
            monitor.distanceOfTimeInWords(slowestEta, moment()).yellow
          }`
        )
    }
  },
  ...
  stop: function(label) {
    ...
    console.log(
      `\n\n${label} completed at ${this.generationEnded.format(
        'YYYY-MM-DD HH:mm:ss'
      )}, took ${timeStr}, results saved to:`
    )
  },
}
...
export default {
  ...
  buildCommand: function(taskStrategyName, phenotype, filename) {
    ...
    var actualRange = monitor.actualRange({
      ...
      days: cmdArgs.days,
      period_length: cmdArgs.period_length,
      min_periods: cmdArgs.min_periods || 1,
    })
  ...
  runCommand: (taskStrategyName, phenotype, command, cb) => {
    ...
    proc.stdout.on('data', (data: string) => {
      if (data.length > 500) {
        ...
        var str = StripAnsi(data.toString()),
          lines = str.split(/(\r?\n)/g)
